...
export default function BiddingTestPage() {
  ...
  const loadServices = async () => {
    try {
      ...
      setError(t('bidding.errorLoadingServices') + ': ' + err.message);
    }
  };
  ...
  const handleCreateBiddingOrder = async (e) => {
    ...
    try {
      ...
      if (!token) {
        setError('❌ ' + t('bidding.mustBeConnected'));
        setLoading(false);
        return;
      }
      ...
      if (response.ok && data.success) {
        setSuccess(`✅ ${t('bidding.orderCreated')} ${data.data.id}`);
        setBidForm(prev => ({ ...prev, order_id: data.data.id }));
        // Recharger les commandes
        loadMyOrders();
      } else {
        setError(data.message || t('bidding.creationError'));
      }
    } catch (err) {
      setError(t('bidding.networkError') + ': ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadMyOrders = async () => {
    ...
    try {
      ...
      setError(t('bidding.errorLoadingOrders') + ': ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateBid = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setError(data.message || t('bidding.errorCreatingOffer'));
      }
    } catch (err) {
      setError(t('bidding.networkError') + ': ' + err.message);
    } finally {
  ...
  const loadOrderBids = async (orderId) => {
    ...
    try {
      ...
      setError(t('bidding.errorLoadingOffers'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAcceptBid = async (bidId) => {
    if (!confirm(t('bidding.confirmAcceptOffer'))) return;
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccess('✅ ' + t('bidding.offerAccepted'));
        ...
        setError(data.message || t('bidding.errorAcceptingOffer'));
      }
    } catch (err) {
      setError(t('bidding.networkError') + ': ' + err.message);
    } finally {
  ...
  if (authLoading) {
    return (
      <div className={styles.container}>
        <div className={styles.loading}>{t('bidding.loading')}</div>
      </div>
    );
  }
    ...
    return (
      <div className={styles.container}>
        <div className={styles.auth}>
          <h2>🔐 {t('bidding.loginRequired')}</h2>
          <p>{t('bidding.loginRequiredDesc')}</p>
          <Button onClick={() => router.push('/login')}>{t('bidding.signIn')}</Button>
