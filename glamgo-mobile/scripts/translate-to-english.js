/**
 * Script de traduction automatique FR -> EN avec DeepL API
...
const EN_FILE = path.join(__dirname, '../src/i18n/translations/en.ts');
...
async function translateText(text) {
  ...
  try {
    ...
    for (const [fr, en] of Object.entries(PRESERVE_TERMS)) {
      // Ne remplacer que si le terme anglais est différent
      if (fr !== en) {
...
/**
 * Génère le contenu du fichier en.ts
 */
function generateEnFile(translations) {
 ...
 */
async function main() {
  console.log('🌍 GlamGo Translation Script (FR → EN)');
  console.log('=====================================\n');
  ...
  const enTranslations = await translateObject(frTranslations);
  const duration = Math.round((Date.now() - startTime) / 1000);

  console.log(`\n\n✅ Translation completed in ${duration} seconds`);

  // Générer le fichier
  console.log('📝 Generating en.ts...');
  const enContent = generateEnFile(enTranslations);
  fs.writeFileSync(EN_FILE, enContent, 'utf-8');

  console.log(`✅ Saved to: ${EN_FILE}`);
